...
def generate_question(artist):
    ...
    # Extract only the track names from the tuples in tracks
    track_names = [track[1] for track in tracks]

    # List of incorrect songs
    incorrect_songs = ["Bohemian Rhapsody", "Like a Rolling Stone", "Imagine", "Billie Jean", "Yesterday", "Hey Jude", "Thriller", "Respect", "Shape of You", "Shallow"]
    incorrect_songs = [song for song in incorrect_songs if song not in track_names]
    
    # Make sure correct answer is not in incorrect answers
    correct_answer_label = correct_answer[1]
    incorrect_songs = [song for song in incorrect_songs if song != correct_answer_label]

    # Combine and shuffle options
    options = [correct_answer_label] + incorrect_songs[:3]
    random.shuffle(options)

    question = f"Which of these tracks is by {artist}?"

    return question, options, correct_answer_label


def generate_album_question(artist_name):
    albums = get_albums_by_artist(artist_name)
    if not albums:
        return "No albums found for this artist", [], None

    # Accessing the album label/name
    correct_album = random.choice(albums)['albumLabel']
...
def generate_genre_question(artist_name):
    ...
    # List of dummy genres for incorrect answers
    incorrect_genres = ["Rock", "Hip Hop", "Jazz", "Classical", "Electronic", "Country", "Reggae", "Blues"]
    incorrect_genres = [genre for genre in incorrect_genres if genre != correct_genre]

    # Select 3 random incorrect genres
    random.shuffle(incorrect_genres)
    incorrect_genres = incorrect_genres[:3]
...
def generate_collaboration_question(artist_name):
    ...
    # You can replace these with more diverse and realistic names
    incorrect_collaborators = ["Jay Z", "50 Cent", "Rhianna", "Shakira", "Sofianne", "Sopranno"]

    # Ensure the correct collaborator is not in the incorrect_collaborators list
    incorrect_collaborators = [collab for collab in incorrect_collaborators if collab != correct_collaborator]

    # Combine correct and incorrect answers
    options = [correct_collaborator] + incorrect_collaborators[:3]
    random.shuffle(options)
...
def format_question(question, options, correct_answer):
    return {
...
def start_quiz(artist_name):
    ...
    questions = {
        "0": get_question(artist_name),
        "1": get_award_question(artist_name),
        # "album_question": get_album_question(artist_name),
        "2": get_question(artist_name),
        "3": get_debut_album_question(artist_name),
        "4": get_genre_question(artist_name),
        "5": get_question(artist_name),
        "6": get_nationality_question(artist_name),
        # "career_question": get_career_question(artist_name),
        "7": get_collaboration_question(artist_name)
    }

    # Shuffle questions
    questions_list = list(questions.items())
    shuffle(questions_list)
    shuffled_questions = dict(questions_list)

    # Write the shuffled questions to a json file
    with open('../threejs/public/quiz.json', 'w') as file:
        json.dump(shuffled_questions, file, indent=4)
